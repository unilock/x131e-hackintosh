 ...
 *
 * Disassembly of iASLyTpFfc.aml, Tue Nov 17 11:53:29 2020
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0000F1FC (61948)
 *     Revision         0x02
 *     Checksum         0x7D
 *     OEM ID           "LENOVO"
...
DefinitionBlock ("", "DSDT", 2, "LENOVO", "IVB-CPT", 0x00000000)
{
    ...
    Scope (_SB)
    {
        ...
        Method (GBIF, 3, NotSerialized)
        {
            If (Arg2)
            {
                If ((BASC != Zero))
                {
                    ...
                    If (Local7)
                    {
                        Arg1 [0x02] = (B1B2 (^PCI0.LPCB.EC.FC00, ^PCI0.LPCB.EC.FC01) * 0x0A)
                    }
                    Else
                    {
                        Arg1 [0x02] = B1B2 (^PCI0.LPCB.EC.FC00, ^PCI0.LPCB.EC.FC01)
                    }

                    SBFC = B1B2 (^PCI0.LPCB.EC.FC00, ^PCI0.LPCB.EC.FC01)
                    ^PCI0.LPCB.EC.HIID = (Arg0 | 0x02)
                    WAEC ()
                    If (Local7)
                    {
                        Local0 = (B1B2 (^PCI0.LPCB.EC.DC00, ^PCI0.LPCB.EC.DC01) * 0x0A)
                    }
                    Else
                    {
                        Local0 = B1B2 (^PCI0.LPCB.EC.DC00, ^PCI0.LPCB.EC.DC01)
                    }

                    SBDC = B1B2 (^PCI0.LPCB.EC.DC00, ^PCI0.LPCB.EC.DC01)
                    Arg1 [One] = Local0
                    Divide (Local0, 0x14, Local1, Arg1 [0x05])
                    Divide (Local0, 0x64, Local1, Arg1 [0x06])
                    Arg1 [0x04] = B1B2 (^PCI0.LPCB.EC.DV00, ^PCI0.LPCB.EC.DV01)
                    SBDV = B1B2 (^PCI0.LPCB.EC.DV00, ^PCI0.LPCB.EC.DV01)
                    Local0 = B1B2 (^PCI0.LPCB.EC.SN00, ^PCI0.LPCB.EC.SN01)
                    SBSN = B1B2 (^PCI0.LPCB.EC.SN00, ^PCI0.LPCB.EC.SN01)
                    ...
                    WAEC ()
                    Arg1 [0x09] = ^PCI0.LPCB.EC.RECB (0xA0, 0x80)
                    SBDN = ^PCI0.LPCB.EC.RECB (0xA0, 0x80)
                    ...
                    BTYP = B1B4 (^PCI0.LPCB.EC.CH00, ^PCI0.LPCB.EC.CH01, ^PCI0.LPCB.EC.CH02, ^PCI0.LPCB.EC.CH03)
                    SBCH = B1B4 (^PCI0.LPCB.EC.CH00, ^PCI0.LPCB.EC.CH01, ^PCI0.LPCB.EC.CH02, ^PCI0.LPCB.EC.CH03)
                    Arg1 [0x0B] = BTYP /* \_SB_.GBIF.BTYP */
                    ^PCI0.LPCB.EC.HIID = (Arg0 | 0x05)
                    WAEC ()
                    Arg1 [0x0C] = ^PCI0.LPCB.EC.RECB (0xA0, 0x80)
                    SBMN = ^PCI0.LPCB.EC.RECB (0xA0, 0x80)
                    BASC = Zero
                    Release (BATM)
                }
                Else
        ...
        Method (GBST, 4, NotSerialized)
        {
            ...
            Else
            {
                Local3 = B1B2 (^PCI0.LPCB.EC.VO00, ^PCI0.LPCB.EC.VO01)
                If (Arg2)
                {
                    Local2 = (B1B2 (^PCI0.LPCB.EC.RC00, ^PCI0.LPCB.EC.RC01) * 0x0A)
                }
                Else
                {
                    Local2 = B1B2 (^PCI0.LPCB.EC.RC00, ^PCI0.LPCB.EC.RC01)
                }

                Local1 = B1B2 (^PCI0.LPCB.EC.AC00, ^PCI0.LPCB.EC.AC01)
                If ((Local1 >= 0x8000))
                {
                    If ((Local0 & One))
                    {
